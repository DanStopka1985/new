...
select coalesce(
               (select concat_ws(' ', sname, fname, mname)
                ...
                where id = (select indiv_id
                              from indiv_code
                              where code = '0b4f89adedb0460731bdb83d1edb038d' and type_id = 1)),
               'indiv not found'
           ) indiv;
...
В Postgres поддерживается несколько видов индексов (методов доступа), а какой vs создали? В нашем скрипте на создание об этом ничего не сказано - только название и поле.
