--получаем случайный идентификатор индивида (просто выбираем случайным образом строку из идентификаторов)
select code
from indiv_code
...
select coalesce(
               (select concat_ws(' ', sname, fname, mname)
                ...
                where id = (select indiv_id
                              from indiv_code
                              where code = '46f174a5b19af3e47ae322a8e0e84a98' and type_id = 1)),
               'indiv not found'
           ) indiv;
...
Как я уже говорил, индекс может ускорить подобный поиск.
...
В Postgres поддерживается несколько видов индексов (методов доступа), а мы какой создали? В нашем скрипте на создание об этом ничего не сказано - только название и поле.
create index if not exists indiv_code_idx on indiv_code(code);
*/
...
Сравним планы (dir compare CTRL+D)
...
Postgres не знает какие запросы мы будем выполнять, по этому по умолчанию разработчики выбрали самый универсальный, который чаще всего работает.
