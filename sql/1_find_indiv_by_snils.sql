...
select coalesce(
               (select concat_ws(' ', sname, fname, mname)
                ...
                where id = (select indiv_id
                              from indiv_code
                              where code = 'b9ccd797d1db6d1b3fa592b179124516' and type_id = 1)),
               'indiv not found'
           ) indiv;
...
Дальше идет сбор собранных данных от всех (Gather).
...
Обратите внимание на то, что стоимость выполнения конечного узла и корневого не сильно отличается.
...
/*
Оказывается, что хэш индекс и места занимает меньше, и если мы будем запускать только этот запрос -
